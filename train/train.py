...
import misc.Img_only as Img_only
...
parser.add_argument('--encoder', default='CAGraph', help='Img_only or CAGraph.')
...
if opt.encoder == 'CAGraph':
    netE = CAGraph._netE(opt.model, opt.ninp, opt.nhid, opt.nlayers, opt.dropout)
elif opt.encoder == 'Img_only':
    netE = Img_only._netE(opt.model, opt.ninp, opt.nhid, opt.nlayers, opt.dropout)
#netE = CAGraph._netE(opt.model, opt.ninp, opt.nhid, opt.nlayers, opt.dropout)
...
def train(epoch):
    ...
    lr = adjust_learning_rate(optimizer, epoch, opt.lr, 10)
...
for epoch in range(opt.start_epoch,opt.niter):
    ...
    print ('%d/%d: NDCG: %f mrr: %.4f R1: %.4f R5 %.4f R10 %.4f Mean %.2f' %(1, len(dataloader_val), ndcg_dic['ndcg'], mrr, R1, R5, R10, ave))
