...
class validate(data.Dataset): # torch wrapper
    def __init__(self, input_val_rcnn, input_ques_h5, input_json, dense_annotations, negative_sample, num_val, data_split, emb_dim=300):
        ...
        f2 = json.load(open(dense_annotations, 'r'))
        self.dense_annotations = f2[:]
    ...
    def __getitem__(self, index):
        ...
        round_id = self.dense_annotations[index]['round_id']
        gt_relevance = np.zeros((100))
        gt_relevance[:] = self.dense_annotations[index]['gt_relevance'][:]
        ...
        for i in range(self.rnd):
            ...
            ans_ids[i] = self.ans_ids[index, i] 
            opt_ids = self.opt_ids[index, i] 
            ans_len[i] = self.ans_len[index, i]
        ...
        ans_len = torch.from_numpy(ans_len)
        opt_ans_len = torch.from_numpy(opt_ans_len)
        gt_relevance = torch.from_numpy(gt_relevance)

        return img, his, ques, ans, ans_target, quesL, opt_ans, \
                    opt_ans_target, ans_ids, ans_len, opt_ans_len, img_id, round_id, gt_relevance


    def __len__(self):
        return self.ques.shape[0]

class train2(data.Dataset): # torch wrapper
    def __init__(self, input_img_rcnn, input_ques_h5, input_json, num_val, data_split, emb_dim=300):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.rnd):
            ...
            opt_ids = self.opt_ids[index, i] # since python start from 0
            gt_ids[i] = self.ans_ids[index, i] # since python start from 0
            # random select the negative samples.
            ans_idx[i] = opt_ids[self.ans_ids[index, i]]
            # exclude the gt index.
            for j, ids in enumerate(opt_ids):
        ...
        return img, his, ques, ans, ans_target, ans_len, ans_idx, ques_ori, \
                opt_ans, opt_ans_len, gt_ids, img_id

    def __len__(self):
        return self.ques.shape[0]

class train_distill(data.Dataset): # torch wrapper
    def __init__(self, input_img_rcnn, input_ques_h5, input_json, negative_sample, num_val, data_split, softlabels, emb_dim=300):
        ...
        print('%s number of data: %d' %(data_split, e-s))
        # load the data.
        f = h5py.File(softlabels, 'r')
        self.softlabels = f['softlabels'][...]
        f.close()
    ...
    def __getitem__(self, index):
        ...
        ans_idx = np.zeros((self.rnd))
        soft_labels = np.zeros((self.rnd, 100))
        ...
        for i in range(self.rnd):
            ...
            # random select the negative samples.
            ans_idx[i] = opt_ids[self.ans_ids[index, i]]
            soft_labels[i,:] = self.softlabels[index, i,:]
            # exclude the gt index.
            for j, ids in enumerate(opt_ids):
                opt_len = self.opt_len[ids]
                opt_ans[i, j, :opt_len] = self.opt_list[ids,:opt_len]
        ...
        return img, his, ques, ans, ans_target, ans_len, ans_idx, ques_ori, \
                opt_ans, opt_ans_len, gt_ids, img_id, soft_labels
