...
class NDCG(object):
    ...
    def observe(self, predicted_scores, target_relevance):
        """
        ...
        predicted_ranks = predicted_ranks.view(-1, 100)
        ...
        for batch_index in range(batch_size):
            ...
            dcg = self._dcg(
                rankings[batch_index][:int(num_relevant)],
                target_relevance[batch_index],
            )
            best_dcg = self._dcg(
                best_rankings[batch_index][:int(num_relevant)],
                target_relevance[batch_index],
            )
    ...
    def _dcg(self, rankings, relevance):
        sorted_tmp = []
        for i in range(len(rankings)):
            sorted_tmp.append(relevance[rankings[i]])
        #pdb.set_trace()
        sorted_relevance = torch.cat(sorted_tmp, dim=-1)
        sorted_relevance = sorted_relevance.data.cpu().float()
        #pdb.set_trace()
        discounts = torch.from_numpy(np.log2((torch.arange(len(rankings)).float() + 2).numpy()))
