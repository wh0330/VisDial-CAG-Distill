...
class _netE(nn.Module):
    ...
    def __init__(self, rnn_type, ninp, nhid, nlayers, dropout):
        ...
        self.ctx_0 = nn.Parameter(torch.randn(1, 1, self.nhid))
    ...
    def forward(self, ques_emb, his_emb, img, ques_hidden, his_hidden, rnd):
        ...
        ####################### graph constrution ############################
        ctx_0 = self.ctx_0.expand_as(rcnn_feat)
        graph = torch.cat((rcnn_feat, ctx_0), dim=2)
        ####################### T == 1 #######################################
        ####################### question command #############################
        ...
        context_1 = self.W8(torch.cat((ctx_0, sum_mes), dim=2))
        ...
        ##################### Multi-modal Fusion ############################
        concat_feat = torch.cat((graph_emb.view(-1, 2*self.nhid), ques_feat.view(-1, self.nhid)),1)
